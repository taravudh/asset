...
const MapView: React.FC<MapViewProps> = ({ projectId }) => {
  const [user, setUser] = useState({ id: 'user123' }); // Placeholder, replace with actual auth hook
  ...
  const [newAssetGeometry, setNewAssetGeometry] = useState<GeoJSON.Geometry | null>(null);
  ...
  const handleCreated = (e: any) => {
    ...
    if (!activeLayerId) {
      ...
      createDefaultLayer(layerTypeForDefault).then(layerId => {
